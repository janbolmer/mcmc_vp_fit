...
h2 = open (path_+'/atoms/h2.dat', 'r')
h2_lines = h2.readlines()
h2.close()

co = open (path_+'/atoms/co.dat', 'r')
co_lines = co.readlines()
co.close()

hd = open (path_+'/atoms/hd.dat', 'r')
hd_lines = hd.readlines()
hd.close()

af = open (path_+'/atoms/atom_excited.dat', 'r')
atom = af.readlines()
af.close()

all_lines_file = open (path_+'/atoms/all_lines.dat', 'r')
all_lines = all_lines_file.readlines()
...
vibH2data = np.genfromtxt(path_+'/atoms/vibH2/' + vibH2[2])
...
class SynSpec(object):
	...
	def add_line(self, spectrum, l, N, b, z):
		'''
		Add line from all_lines.dat
		'''
			...
			if str(a[0]) == str(l):
				if min(wav_range) < float(a[1])*(1+redshift) < max(wav_range):
					if check_lst(ignore_lst, float(a[1])) == 0.0:
						...
						f = float(a[2])
						gamma = float(a[3])
						spec *= addAbs(wav_range, N, lamb, f, gamma, b, \
							z,res=self.resolution)
		return spec
