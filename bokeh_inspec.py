...
def bokeh_inspec(file, redshift):
	...
	co_name, co_wav = get_lines(redshift)

	wav_aa, flux, flux_err, qual, cont, \
	cont_err, flux_corr, flux_err_corr, \
	n_flux, n_flux_err, \
	n_flux2, n_flux_err2 = \
	get_data2(file, redshift, wl_range=False)

	output_file("spec_bokeh.html",
	...
	p = bokeh_fig(title="X-shooter spectrum of " + "at z = " +
	...
	for i in np.arange(0, len(co_name), 1):
		...
		vline = Span(location=co_wav[i], dimension='height',
			line_color='green', line_width=1, line_dash='dashed')

		if i%2 == 0:
			atom_label = Label(x=co_wav[i]+0.2, y=1.4,
				text=a_name[i]+" " + str(round(a_wav[i]/(1+redshift), 2)),
				text_font_size="8pt", text_color="cyan",
				text_font="helvetica", angle=0.6)
		else:
			atom_label = Label(x=co_wav[i]+0.2, y=1.7,
				text=a_name[i]+" " + str(round(a_wav[i]/(1+redshift), 2)),
				text_font_size="8pt", text_color="cyan",
				text_font="helvetica", angle=0.6)			
	...
	p.line(x="wav_aa", y="n_flux", source=source,
		legend=file.strip('specta/')[0:-10] + "Data", line_width=2,
		color="black")
