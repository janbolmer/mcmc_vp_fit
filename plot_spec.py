...
def get_spec(file):
	...
	if not file.startswith("spectra/GRB180205A"):
		...
		for line in data:
			if not line.startswith(("#", "GRB", "Resolution", "PSFFWHM")):
				wav_aa = np.append(wav_aa,float(line.split()[1]))
				flux = np.append(flux,float(line.split()[2]))
				flux_err = np.append(flux_err,float(line.split()[3]))
				model = np.append(model,float(line.split()[2]))
			if line.startswith('GRB'):
				grb_name = str(line.split()[0]).split("_")[0]
			if line.startswith('Res'):
				res = float(line.split()[1])
...
def main():
	...
	if not spec_file.startswith("spectra/GRB180205A"):
